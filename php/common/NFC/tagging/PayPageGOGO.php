...
if($total_record!=1) {
	echo "{\"status\":\"NO\",\"num_results\":\"$total_record\"}";
}
else {
if($stock-$prcnt<0 || $prcnt==0) {
	...
	$sql ="UPDATE TB_PRODUCTS SET PR_STOCK=PR_STOCK-'$prcnt' where PR_KEY='$now_key'";
        ...
        $sql ="SELECT * FROM TB_CART where CA_PRKEY = '$now_key' and CA_USID='$userid' and CA_PAID=1";
        $result = mysql_query($sql, $connect);	
        $total_record = mysql_num_rows($result);

        //구매한 경험이 있으면
        if($total_record >0) {
              $sql="UPDATE TB_CART SET CA_PRCNT=CA_PRCNT+'$prcnt' where CA_PRKEY='$now_key' and CA_USID='$userid' and CA_PAID=1";
              mysql_query($sql, $connect);

        }
        else{
                $sql="INSERT INTO TB_CART(CA_USID, CA_PRCNT, CA_PAID, CA_PRKEY) VALUES('$userid', '$prcnt', 1, '$now_key')";
                mysql_query($sql, $connect);
        }
